...
export default function Home() {
  ...
  const [opportunityFeedOpen, setOpportunityFeedOpen] = useState(() => typeof window !== "undefined" && new URLSearchParams(window.location.search).get("workspace") === "opportunity-feed");
  ...
  const openDecisionCenterWorkspace = () => { setPaperOpen(false); setSetupMode(null); setSetupMonitorOpen(false); setWatchlistOpen(false); setAutoPaperOpen(false); setTradingIntelligenceOpen(false); setDiscoveryOpen(false); setOpportunityFeedOpen(false); setDecisionCenterOpen(true); setActiveNav("Decision Center"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname === "/decision" ? "/decision" : "/"}?workspace=decision-center`); };
