...
function StatusDot({ status }: { status: "live" | "stale" | "unavailable" }) {
  ...
  const effectiveStatus = online ? status : "stale";
...
export default function Home() {
  const { online, updateReady, markReadSnapshot } = usePwaStatus();
  ...
  useEffect(() => { if (online) markReadSnapshot(scan?.generatedAt); }, [online, scan?.generatedAt, markReadSnapshot]);
  useEffect(() => { if (selectedId) setAssetIntelligenceOpen(true); }, [selectedId]);
  const openPaperWorkspace = (asset: ScannerRow | null) => { setPaperAsset(asset); setActiveNav("Paper Trading"); setPaperOpen(true); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=paper-trading`); };
  const closePaperWorkspace = () => { setPaperOpen(false); setActiveNav("Dashboard"); if (typeof window !== "undefined") window.history.replaceState(null, "", window.location.pathname); };
  ...
  useEffect(() => {
    const onMobileNavigate = (event: Event) => {
      const target = (event as CustomEvent<"dashboard" | "scanner" | "paper" | "alerts" | "research">).detail;
      if (target === "paper") openPaperWorkspace(selected);
        ...
        else { setActiveNav(target === "scanner" ? "Market Scanner" : "Dashboard"); }
      }
    };
  ...
  return <div className={cn("min-h-screen bg-[#060a12] text-slate-100 selection:bg-cyan-300/30", (!online || updateReady) && "pt-10")}>
    <AssetIntelligencePanel row={selected} open={assetIntelligenceOpen} onOpenChange={setAssetIntelligenceOpen} onPaperTrade={row => { setAssetIntelligenceOpen(false); openPaperWorkspace(row); }} />
