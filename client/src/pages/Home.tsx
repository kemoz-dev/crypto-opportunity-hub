...
function AutoPaperSummaryCard({ authenticated, onOpen }: { authenticated: boolean; onOpen: () => void }) { const settings = trpc.crypto.autoPaperSettings.useQuery(undefined, { enabled: authenticated, staleTime: 30_000 }); const performance = trpc.crypto.autoPaperPerformance.useQuery(undefined, { enabled: authenticated, staleTime: 30_000 }); if (!authenticated) return <section className="mb-5 rounded-xl border border-white/[.07] bg-white/[.02] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-cyan-200">Auto Paper</p><p className="mt-1 text-xs text-slate-500">Sign in to view your private simulation summary.</p></div><button onClick={onOpen} className="rounded-md border border-cyan-300/20 bg-cyan-300/[.06] px-3 py-2 text-[11px] text-cyan-100">Open Lab</button></div></section>; return <button type="button" onClick={onOpen} className="mb-5 w-full rounded-xl border border-cyan-300/15 bg-cyan-300/[.03] p-4 text-left transition-colors hover:bg-cyan-300/[.06]"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-cyan-200">Auto Paper</p><p className="mt-1 text-sm font-semibold text-slate-100">{settings.data?.enabled ? "ON · simulating eligible setups" : "OFF · simulation disabled"}</p></div><span className="rounded-full border border-emerald-300/20 bg-emerald-300/[.06] px-2 py-1 text-[10px] text-emerald-100">SIMULATED ONLY</span></div><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5"><span className="rounded-lg border border-white/[.06] p-2"><small className="block text-[9px] uppercase text-slate-500">Active</small><strong>{performance.data?.active ?? "—"}</strong></span><span className="rounded-lg border border-white/[.06] p-2"><small className="block text-[9px] uppercase text-slate-500">Today’s entries</small><strong>Persisted only</strong></span><span className="rounded-lg border border-white/[.06] p-2"><small className="block text-[9px] uppercase text-slate-500">Wins</small><strong>{performance.data?.wins ?? "—"}</strong></span><span className="rounded-lg border border-white/[.06] p-2"><small className="block text-[9px] uppercase text-slate-500">Losses</small><strong>{performance.data?.losses ?? "—"}</strong></span><span className="rounded-lg border border-white/[.06] p-2"><small className="block text-[9px] uppercase text-slate-500">Simulated P/L</small><strong>{performance.data?.simulatedPnl == null ? "—" : performance.data.simulatedPnl.toFixed(2)}</strong></span></div></button>; }
...
export default function Home() {
  ...
  const openWatchlistWorkspace = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setSetupMonitorOpen(false); setAutoPaperOpen(false); setWatchlistOpen(true); setActiveNav("Watchlist"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=watchlist`); };
  const openAutoPaperWorkspace = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setSetupMonitorOpen(false); setWatchlistOpen(false); setAutoPaperOpen(true); setActiveNav("Auto Paper Lab"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=auto-paper`); };
  ...
  useEffect(() => {
    const onMobileNavigate = (event: Event) => {
      const target = (event as CustomEvent<"dashboard" | "scanner" | "discovery" | "setup-monitor" | "watchlist" | "scalping" | "swing" | "paper" | "auto-paper" | "alerts" | "research">).detail;
  ...
  return <div className={cn("min-h-screen bg-[#060a12] text-slate-100 selection:bg-cyan-300/30", pwaBannerVisible && "pt-16")}>
    <Suspense fallback={null}><LazyAssetIntelligencePanel row={selected} open={assetIntelligenceOpen} onOpenChange={setAssetIntelligenceOpen} onPaperTrade={(row, mode) => { setAssetIntelligenceOpen(false); openPaperWorkspace(row, mode); }} />
