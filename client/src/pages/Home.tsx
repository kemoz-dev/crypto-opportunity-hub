...
export default function Home() {
  ...
  const closeWorkspaceOverlays = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setOpportunityFeedOpen(false); setSetupMonitorOpen(false); setWatchlistOpen(false); setAutoPaperOpen(false); setTradingIntelligenceOpen(false); setActiveNav("Home"); if (typeof window !== "undefined") window.history.replaceState(null, "", window.location.pathname); };
  ...
  useEffect(() => {
    const onMobileNavigate = (event: Event) => {
      const target = (event as CustomEvent<"dashboard" | "scanner" | "discovery" | "opportunity-feed" | "setup-monitor" | "watchlist" | "scalping" | "swing" | "paper" | "auto-paper" | "alerts" | "research" | "trading-intelligence" | "settings">).detail;
