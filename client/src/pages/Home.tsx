...
export default function Home() {
  ...
  const openSetupWorkspace = (mode: "SCALP" | "SWING") => { setPaperOpen(false); setDiscoveryOpen(false); setSetupMode(mode); setActiveNav(mode === "SCALP" ? "Scalp" : "Swing"); };
  const openDiscoveryWorkspace = () => { setPaperOpen(false); setSetupMode(null); setSetupMonitorOpen(false); setDiscoveryOpen(true); setActiveNav("Opportunities"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=opportunity-discovery`); };
  const openSetupMonitorWorkspace = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setWatchlistOpen(false); setSetupMonitorOpen(true); setActiveNav("Monitor"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=setup-monitor`); };
  ...
  const closeWorkspaceOverlays = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setSetupMonitorOpen(false); setWatchlistOpen(false); setAutoPaperOpen(false); setActiveNav("Home"); if (typeof window !== "undefined") window.history.replaceState(null, "", window.location.pathname); };
  ...
  useEffect(() => {
    const onMobileNavigate = (event: Event) => {
      const target = (event as CustomEvent<"dashboard" | "scanner" | "discovery" | "setup-monitor" | "watchlist" | "scalping" | "swing" | "paper" | "auto-paper" | "alerts" | "research" | "settings">).detail;
