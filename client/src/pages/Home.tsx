...
export default function Home() {
  ...
  const openSetupWorkspace = (mode: "SCALP" | "SWING") => { setPaperOpen(false); setDiscoveryOpen(false); setSetupMonitorOpen(false); setWatchlistOpen(false); setAutoPaperOpen(false); setTradingIntelligenceOpen(false); setSetupMode(mode); setActiveNav(mode === "SCALP" ? "Scalp" : "Swing"); };
  const openDiscoveryWorkspace = () => { setPaperOpen(false); setSetupMode(null); setSetupMonitorOpen(false); setWatchlistOpen(false); setAutoPaperOpen(false); setTradingIntelligenceOpen(false); setDiscoveryOpen(true); setActiveNav("Opportunities"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=opportunity-discovery`); };
  const openSetupMonitorWorkspace = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setWatchlistOpen(false); setAutoPaperOpen(false); setTradingIntelligenceOpen(false); setSetupMonitorOpen(true); setActiveNav("Monitor"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=setup-monitor`); };
  const openWatchlistWorkspace = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setSetupMonitorOpen(false); setAutoPaperOpen(false); setTradingIntelligenceOpen(false); setWatchlistOpen(true); setActiveNav("Watchlist"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=watchlist`); };
  const openAutoPaperWorkspace = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setSetupMonitorOpen(false); setWatchlistOpen(false); setTradingIntelligenceOpen(false); setAutoPaperOpen(true); setActiveNav("Auto Paper Lab"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=auto-paper`); };
  const openTradingIntelligenceWorkspace = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setSetupMonitorOpen(false); setWatchlistOpen(false); setAutoPaperOpen(false); setTradingIntelligenceOpen(true); setActiveNav("Trading Intelligence"); if (typeof window !== "undefined") window.history.replaceState(null, "", `${window.location.pathname}?workspace=trading-intelligence`); };
  const closeWorkspaceOverlays = () => { setPaperOpen(false); setSetupMode(null); setDiscoveryOpen(false); setSetupMonitorOpen(false); setWatchlistOpen(false); setAutoPaperOpen(false); setTradingIntelligenceOpen(false); setActiveNav("Home"); if (typeof window !== "undefined") window.history.replaceState(null, "", window.location.pathname); };
  ...
  useEffect(() => {
    const onMobileNavigate = (event: Event) => {
      const target = (event as CustomEvent<"dashboard" | "scanner" | "discovery" | "setup-monitor" | "watchlist" | "scalping" | "swing" | "paper" | "auto-paper" | "alerts" | "research" | "trading-intelligence" | "settings">).detail;
