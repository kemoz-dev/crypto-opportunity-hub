...
import {
  Activity, AlertTriangle, ArrowDownRight, ArrowUpRight, BarChart3, BellRing, BookOpen, Calculator, ChevronDown,
...
export default function Home() {
  ...
  return <div className="min-h-screen bg-[#060a12] text-slate-100 selection:bg-cyan-300/30">
    ...
    <div className="relative mx-auto flex min-h-screen max-w-[1800px]">
      ...
      </main></div><DetailPanel row={selected} onClose={() => setSelectedId(null)} onPaperTrade={row => { setPaperAsset(row); setPaperOpen(true); }} /><SettingsPanel open={settingsOpen} onOpenChange={setSettingsOpen} /><PaperTradingPanel open={paperOpen} onOpenChange={setPaperOpen} asset={paperAsset} /><BacktestingPanel open={backtestingOpen} onOpenChange={setBacktestingOpen} selectedAsset={selected} /><AlertsPanel open={alertsOpen} onOpenChange={setAlertsOpen} /><ResearchSummaryPanel open={researchSummaryOpen} onOpenChange={setResearchSummaryOpen} /><OpportunityResearchLab open={researchLabOpen} onOpenChange={setResearchLabOpen} /><HistoricalDataFoundation open={historicalDataOpen} onOpenChange={setHistoricalDataOpen} /><ExecutionCostLab open={executionCostLabOpen} onOpenChange={setExecutionCostLabOpen} />
    </div>
  </div>;
}
