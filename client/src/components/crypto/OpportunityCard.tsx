...
export type OpportunityCardData = {
  ...
  quality?: { level?: string | null; score?: number | null; confidence?: string | null } | null;
  health?: string | null;
  why?: string | null;
  reasons?: string[];
  ...
  risk?: string | null;
  warning?: string | null;
  autoPaperState?: string | null;
  autoPaperReason?: string | null;
};
...
function Evidence({ label, value, tone, icon }: { label: string; value: string; tone: "cyan" | "rose" | "amber" | "slate"; icon?: ReactNode }) {
  const toneClass = { cyan: "border-cyan-300/15 bg-cyan-300/[.035] text-cyan-100", rose: "border-rose-300/15 bg-rose-300/[.035] text-rose-100", amber: "border-amber-300/15 bg-amber-300/[.035] text-amber-100", slate: "border-white/[.07] bg-white/[.02] text-slate-300" }[tone];
  return <div className={cn("min-w-0 rounded-lg border p-2.5", toneClass)}><div className="flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[.14em] opacity-70">{icon}{label}</div><p className="mt-1 text-[11px] leading-4">{value}</p></div>;
}
...
export function OpportunityCard({ data, onView, onMonitor, onPaper, paperDisabled = false }: { data: OpportunityCardData; onView?: () => void; onMonitor?: () => void; onPaper?: () => void; paperDisabled?: boolean }) {
  ...
  const hasStop = data.stop?.price != null;
  const hasRewardRisk = data.rewardRisk != null;
  const hasPlanEvidence = hasEntry || hasStop || hasRewardRisk || targets.length > 0;
  const isUnavailable = data.status === "DATA UNAVAILABLE";
  const displayStatus = isUnavailable ? "DATA LIMITED" : data.status;
  const stateMessage = displayStatus === "POTENTIAL" ? "POTENTIAL — REVIEW REQUIRED" : displayStatus === "WATCH" ? "WATCH — MONITOR EVIDENCE" : displayStatus === "NO TRADE" ? "NO TRADE — DIAGNOSTIC STATE" : displayStatus === "DATA LIMITED" ? "DATA LIMITED — PLAN NOT VALIDATED" : displayStatus;
  const planUnavailable = !hasPlanEvidence || isUnavailable;
  const reasons = data.reasons?.slice(0, 3) ?? [];
  const riskMessage = data.risk ?? (data.health && data.health !== "HEALTHY" ? data.health : hasRewardRisk ? `Validated R:R ${data.rewardRisk!.toFixed(2)}` : "No separate risk assessment returned.");
  ...
  return <article className="group flex min-w-0 flex-col rounded-2xl border border-white/[.08] bg-[#0a111e]/90 p-4 shadow-[0_12px_35px_rgba(2,8,23,.12)] transition-colors hover:border-cyan-300/20">
    <div className="flex items-start justify-between gap-3">
      ...
      <div className="flex shrink-0 flex-col items-end gap-1"><span role="status" className={cn("rounded-full border px-2 py-1 text-[10px] font-semibold uppercase tracking-[.08em]", statusTone(data.status))}>{displayStatus}</span><span className="max-w-36 text-right text-[9px] font-semibold uppercase tracking-[.08em] text-slate-500">{stateMessage}</span></div>
    </div>
    <div className="mt-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-5">
      ...
      <div className="rounded-lg border border-white/[.06] bg-white/[.02] p-2"><p className="text-[9px] uppercase tracking-[.12em] text-slate-500">Quality</p><p className="mt-1 text-[10px] font-semibold uppercase text-cyan-100">{data.quality?.level ?? "UNAVAILABLE"}</p>{data.quality?.confidence ? <p className="mt-1 text-[9px] text-slate-500">{data.quality.confidence}</p> : null}</div>
    ...
    <div className="mt-4 grid gap-3 border-t border-white/[.06] pt-3 text-xs sm:grid-cols-[1fr_auto]">
      <div className="min-w-0">
        {planUnavailable ? <div className="rounded-lg border border-slate-400/15 bg-slate-400/[.04] p-3"><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-slate-300">PLAN UNAVAILABLE</p><p className="mt-1 text-[11px] leading-4 text-slate-500">No valid level is returned by the server. Entry, Stop Loss, TP1/TP2/TP3, and R:R remain hidden until a valid plan is returned.</p></div> : <><div className="flex flex-wrap gap-x-4 gap-y-2"><span><strong className="mr-1 text-[9px] uppercase tracking-[.12em] text-slate-500">Entry</strong>{hasEntry ? `${price(entry.low)} – ${price(entry.high)}` : "Unavailable"}</span>{hasStop ? <span><strong className="mr-1 text-[9px] uppercase tracking-[.12em] text-slate-500">Stop Loss</strong>{price(data.stop?.price)}</span> : null}{hasRewardRisk ? <span><strong className="mr-1 text-[9px] uppercase tracking-[.12em] text-slate-500">R:R</strong>{data.rewardRisk!.toFixed(2)}</span> : null}</div>{targets.length ? <div className="mt-2 flex flex-wrap gap-2"><strong className="text-[9px] uppercase tracking-[.12em] text-slate-500">Targets</strong>{targets.map((target, index) => <span key={`${target.label ?? "TP"}-${index}`} className={cn("font-mono", target.status === "REACHED" ? "text-emerald-300" : "text-slate-300")}>{target.label ?? `TP${index + 1}`} {price(target.price)}{target.status === "REACHED" ? " · reached" : ""}</span>)}</div> : null}{data.invalidationDistance != null ? <p className="mt-2 text-[10px] text-slate-500">Distance to invalidation: <span className="font-mono text-slate-300">{data.invalidationDistance.toFixed(2)}%</span></p> : null}</>}
      </div>
      <div className="flex flex-wrap items-end justify-end gap-2"><Button type="button" variant="ghost" size="sm" onClick={onView} disabled={!onView} className="min-h-11 px-2 text-[11px] text-slate-300"><Eye className="mr-1.5 h-3.5 w-3.5" />View Asset</Button>{data.canMonitor && onMonitor ? <Button type="button" variant="outline" size="sm" onClick={onMonitor} className="min-h-11 border-cyan-300/20 px-2 text-[11px] text-cyan-100"><Target className="mr-1.5 h-3.5 w-3.5" />Monitor Setup</Button> : null}{onPaper ? <Button type="button" size="sm" onClick={onPaper} disabled={paperDisabled} className="min-h-11 bg-emerald-300 px-2 text-[11px] text-slate-950 hover:bg-emerald-200 disabled:bg-slate-700"><ArrowRight className="mr-1.5 h-3.5 w-3.5" />Paper Trade</Button> : null}</div>
    </div>
    <div className="mt-4 grid gap-2 border-t border-white/[.06] pt-3 sm:grid-cols-2"><Evidence label="WHY" value={data.why ?? (isUnavailable ? "Required validated source data is unavailable." : "Server-derived evidence is available in the detail workspace.")} tone="cyan" icon={isUnavailable ? <ShieldAlert className="h-3.5 w-3.5" /> : <Gauge className="h-3.5 w-3.5" />} /><Evidence label="RISK" value={riskMessage} tone="rose" /><Evidence label="WARNING" value={warningMessage} tone="amber" /><Evidence label="DATA" value={`${data.provider ?? "Provider unavailable"} · ${data.freshness ?? (data.dataTimestamp ? timeAgo(data.dataTimestamp) : "Freshness unavailable")} · ${data.dataQuality ?? "Quality unavailable"}`} tone="slate" />{data.autoPaperState ? <Evidence label="AUTO PAPER · SIMULATION EVIDENCE" value={`${data.autoPaperState}${data.autoPaperReason ? ` · ${data.autoPaperReason}` : ""}`} tone="amber" /> : null}{onView ? <button type="button" onClick={onView} className="col-span-full flex min-h-11 items-center justify-end gap-1 text-[10px] font-semibold uppercase tracking-[.12em] text-cyan-200 hover:text-cyan-100">Open asset workspace <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" /></button> : null}</div>
