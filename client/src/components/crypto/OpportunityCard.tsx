...
export type OpportunityCardData = {
  ...
  targets?: Array<{ label?: string | null; price?: number | null }>;
...
export function OpportunityCard({ data, onView, onMonitor, onPaper, paperDisabled = false }: { data: OpportunityCardData; onView?: () => void; onMonitor?: () => void; onPaper?: () => void; paperDisabled?: boolean }) {
  ...
  return <article className="group flex min-w-0 flex-col rounded-2xl border border-white/[.08] bg-[#0a111e]/90 p-4 transition-colors hover:border-cyan-300/20">
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate text-base font-semibold text-slate-100">{data.symbol}</h3>{data.setupType ? <span className="text-[10px] uppercase tracking-[.14em] text-slate-500">{data.setupType}</span> : null}</div>{data.name ? <p className="mt-1 truncate text-[11px] text-slate-500">{data.name}</p> : null}</div>
      <span className={cn("shrink-0 rounded-full border px-2 py-1 text-[10px] font-semibold uppercase tracking-[.08em]", statusTone(data.status))}>{statusLabel(data.status)}</span>
    </div>
    ...
    <div className="mt-4 grid gap-3 border-t border-white/[.06] pt-3 text-xs sm:grid-cols-[1fr_auto]">
      <div className="min-w-0"><div className="flex flex-wrap gap-x-4 gap-y-2"><span><strong className="mr-1 text-[9px] uppercase tracking-[.12em] text-slate-500">Entry</strong>{entry?.low != null && entry.high != null ? `${price(entry.low)} – ${price(entry.high)}` : "No valid level"}</span><span><strong className="mr-1 text-[9px] uppercase tracking-[.12em] text-slate-500">Stop</strong>{data.stop?.price != null ? price(data.stop.price) : "No valid level"}</span></div><div className="mt-2 flex flex-wrap gap-2"><strong className="text-[9px] uppercase tracking-[.12em] text-slate-500">Targets</strong>{targets.length ? targets.map((target, index) => <span key={`${target.label ?? "TP"}-${index}`} className="font-mono text-slate-300">{target.label ?? `TP${index + 1}`} {price(target.price)}</span>) : <span className="text-slate-500">No valid level</span>}</div></div>
    ...
    <div className={cn("mt-3 flex gap-2 text-xs leading-5", isUnavailable ? "text-slate-400" : "text-slate-300")}><span className="mt-0.5 shrink-0 text-cyan-200">{isUnavailable ? <ShieldAlert className="h-3.5 w-3.5" /> : <Gauge className="h-3.5 w-3.5" />}</span><span><strong className="text-[9px] uppercase tracking-[.12em] text-slate-500">Why</strong> {data.why ?? (isUnavailable ? "Required validated source data is unavailable." : "Server-derived evidence is available in the detail workspace.")}</span>{onView ? <ArrowRight className="ml-auto mt-1 h-3.5 w-3.5 shrink-0 text-slate-600 transition-transform group-hover:translate-x-0.5" /> : null}</div>
  </article>;
}
