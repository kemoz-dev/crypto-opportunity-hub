...
type FeedItem = {
  ...
  opportunityQuality?: { level?: string | null; score?: number | null; confidence?: string | null; explanation?: string } | null;
  ...
  sourcePlan?: { entryZone?: { preferred: number } | null; stop?: { price: number } | null; targets?: Array<{ label: string; price: number }> ; rewardRisk?: number | null; risks?: string[] };
  strategy: Strategy;
  sector: string;
};
type FeedDiscoverySummary = { evaluated: number; qualified: number; potential: number; watch: number; noTrade: number; dataUnavailable: number; restrictedByRiskOff: number };
type FeedDiagnosticSummary = { evaluatedAssets: number; noTradeAssets: number; byCondition: Array<{ key: string; label: string; passed: number; failed: number; unavailable: number; stale: number }>; topNoTradeReasons: Array<{ key: string; label: string; count: number; status: "FAILED" | "UNAVAILABLE" | "STALE" }>; classification: { lackOfMarketSetups: number; missingData: number; staleData: number; existingSetupRequirement: number } };
type FeedProviderHealth = { provider?: string; status?: string; lastSuccessfulAt?: number | null; lastFailureAt?: number | null; supportedTimeframes?: string[]; validatedSymbols?: string[]; failureClassification?: string | null };
type DiscoveryResponse = { mode?: Strategy; generatedAt?: number; lowerTimeframeDataReady?: boolean; minimumValidatedTimeframe?: string; marketRegime?: { classification?: string | null; score?: number | null } | null; providerHealth?: FeedProviderHealth[]; diagnostics?: FeedDiagnosticSummary; discovery?: { items?: FeedItem[]; summary?: FeedDiscoverySummary } };
...
function toCard(item: FeedItem, eligibilityResponse: EligibilityResponse | undefined, autoPaperEnabled: boolean | undefined): OpportunityCardData {
  ...
  return {
    ...
    autoPaperState: autoPaperEnabled == null ? "AUTHENTICATION REQUIRED" : autoPaperEnabled ? eligibility?.state ?? "NO MATCHING SAMPLE" : "AUTO PAPER OFF", autoPaperReason: autoPaperEnabled ? eligibility?.reason ?? eligibility?.warning ?? (eligibility ? null : "No matching persisted simulation sample was returned.") : "Simulation remains disabled until explicitly enabled by the owner.",
...
export function OpportunityFeedWorkspace({ onBack, onInspectAsset, onOpenDecision }: { onBack: () => void; onInspectAsset: (assetId: string, returnTo?: string) => void; onOpenDecision: (assetId?: string, returnTo?: string) => void }) {
  ...
  const [quickViewItem, setQuickViewItem] = useState<FeedItem | null>(null);
  ...
  const serverSummary = useMemo(() => {
    const summaries = [scalp?.discovery?.summary, swing?.discovery?.summary].filter((summary): summary is FeedDiscoverySummary => Boolean(summary));
    if (!summaries.length) return null;
    return summaries.reduce((total, summary) => ({
      evaluated: total.evaluated + summary.evaluated,
      qualified: total.qualified + summary.qualified,
      potential: total.potential + summary.potential,
      watch: total.watch + summary.watch,
      noTrade: total.noTrade + summary.noTrade,
      dataUnavailable: total.dataUnavailable + summary.dataUnavailable,
      restrictedByRiskOff: total.restrictedByRiskOff + summary.restrictedByRiskOff,
    }), { evaluated: 0, qualified: 0, potential: 0, watch: 0, noTrade: 0, dataUnavailable: 0, restrictedByRiskOff: 0 });
  }, [scalp?.discovery?.summary, swing?.discovery?.summary]);
  const serverNoQualifiedReasons = useMemo(() => {
    const reasonMap = new Map<string, { label: string; count: number; status: string }>();
    for (const diagnostics of [scalp?.diagnostics, swing?.diagnostics]) {
      for (const reason of diagnostics?.topNoTradeReasons ?? []) {
        const current = reasonMap.get(reason.key);
        reasonMap.set(reason.key, { label: reason.label, count: (current?.count ?? 0) + reason.count, status: reason.status });
      }
    }
    return Array.from(reasonMap.values()).sort((left, right) => right.count - left.count || left.label.localeCompare(right.label)).slice(0, 5);
  }, [scalp?.diagnostics, swing?.diagnostics]);
  const potentialDiagnostics = useMemo(() => items.filter(item => item.status === "POTENTIAL").slice(0, 4), [items]);
  ...
  const filterSummary = useMemo(() => {
    const chips: Array<{ key: string; label: string; patch: Partial<FeedFilters> }> = [];
    if (feedFilters.status !== "ALL") chips.push({ key: "status", label: feedFilters.status, patch: { status: "ALL" } });
    if (feedFilters.strategy !== "ALL") chips.push({ key: "strategy", label: feedFilters.strategy, patch: { strategy: "ALL" } });
    if (feedFilters.direction !== "ALL") chips.push({ key: "direction", label: feedFilters.direction, patch: { direction: "ALL" } });
    if (feedFilters.health !== "ALL") chips.push({ key: "health", label: feedFilters.health, patch: { health: "ALL" } });
    if (feedFilters.regime !== "ALL") chips.push({ key: "regime", label: feedFilters.regime, patch: { regime: "ALL" } });
    if (feedFilters.dataLimited) chips.push({ key: "data", label: "DATA LIMITED", patch: { dataLimited: false } });
    if (feedFilters.watchlistOnly) chips.push({ key: "watchlist", label: "WATCHLIST ONLY", patch: { watchlistOnly: false } });
    return chips;
  }, [feedFilters]);
  ...
  return <section className="min-h-screen pb-24 rounded-2xl border border-white/[.08] bg-[#080e19]/95 p-4 text-slate-100 shadow-2xl sm:p-6 lg:pb-6">
    ...
    {serverSummary ? <section className="mt-5 rounded-2xl border border-cyan-300/15 bg-cyan-300/[.025] p-4" aria-label="Live Opportunity Summary"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-cyan-200">LIVE OPPORTUNITY SUMMARY</p><h2 className="mt-1 text-sm font-semibold text-slate-100">Server-reported opportunity states</h2></div><span className="rounded-full border border-cyan-300/20 bg-cyan-300/[.06] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[.12em] text-cyan-100">SERVER DERIVED</span></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-7"><SummaryMetric label="Opportunities" value={serverSummary.evaluated} /><SummaryMetric label="Qualified" value={serverSummary.qualified} tone="positive" /><SummaryMetric label="Potential" value={serverSummary.potential} tone="amber" /><SummaryMetric label="Watch" value={serverSummary.watch} tone="cyan" /><SummaryMetric label="No Trade" value={serverSummary.noTrade} tone="risk" /><SummaryMetric label="Data Limited" value={serverSummary.dataUnavailable} tone="muted" /><SummaryMetric label="Risk Off restricted" value={serverSummary.restrictedByRiskOff} tone="risk" /></div></section> : null}
    {serverSummary && serverSummary.qualified === 0 ? <section className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/[.035] p-4" aria-label="Why no qualified trades"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-amber-200">WHY NO QUALIFIED TRADES?</p><p className="mt-1 text-xs text-slate-400">Only server-returned diagnostic reasons are shown; no client-side reason was created.</p></div><span className="rounded-full border border-amber-300/20 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[.12em] text-amber-100">QUALIFIED 0</span></div>{serverNoQualifiedReasons.length ? <div className="mt-3 flex flex-wrap gap-2">{serverNoQualifiedReasons.map(reason => <span key={reason.label} className="rounded-lg border border-white/[.1] bg-black/10 px-3 py-2 text-[11px] text-slate-300">{reason.label} · {reason.count} {reason.status.toLowerCase()}</span>)}</div> : <p className="mt-3 rounded-lg border border-dashed border-white/[.1] p-3 text-xs text-slate-500">Server diagnostic reasons are currently UNAVAILABLE.</p>}</section> : null}
    {potentialDiagnostics.length ? <section className="mt-5 rounded-2xl border border-violet-300/15 bg-violet-300/[.025] p-4" aria-label="Potential setup diagnostics"><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-violet-200">POTENTIAL SETUPS</p><p className="mt-1 text-xs text-slate-400">What is missing is read directly from each server-returned setup.</p></div><div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{potentialDiagnostics.map(item => <article key={`${item.strategy}-${item.assetId}`} className="rounded-xl border border-white/[.08] bg-black/10 p-3"><div className="flex items-center justify-between gap-2"><strong className="text-sm text-slate-100">{item.symbol}</strong><span className="rounded-full border border-amber-300/20 px-2 py-1 text-[9px] font-semibold uppercase text-amber-100">POTENTIAL — REVIEW REQUIRED</span></div><p className="mt-2 text-[10px] uppercase tracking-[.1em] text-slate-500">Quality / missing evidence</p><p className="mt-1 text-xs leading-5 text-slate-300">{item.missingEvidence?.length ? item.missingEvidence.join(" · ") : item.confirmationRequirements?.length ? item.confirmationRequirements.join(" · ") : "UNAVAILABLE"}</p><p className="mt-2 text-[10px] text-slate-500">{item.strategy} · {String(item.timeframes?.execution ?? "UNAVAILABLE").toUpperCase()} · Data {item.freshness ?? "UNAVAILABLE"}</p></article>)}</div></section> : null}
    {!scalpQuery.isLoading && !swingQuery.isLoading && !scalpQuery.error && !swingQuery.error && (!hasActiveFeedFilters(feedFilters) || filteredItems.length > 0) ? <FeedSection title="TOP OPPORTUNITIES" subtitle="Highest current server-ranked Qualified and Potential states; display filters never re-score." items={topOpportunities} empty="NO CURRENT OPPORTUNITIES" onInspectAsset={onInspectAsset} onOpenDecision={onOpenDecision} onToggleWatchlist={toggleWatchlist} watchedAssetIds={watchedAssetIds} watchlistPending={addWatch.isPending || removeWatch.isPending} returnTo={returnContext} eligibility={eligibility} autoPaperEnabled={isAuthenticated ? settingsQuery.data?.enabled : undefined} onQuickView={setQuickViewItem} /> : null}
    <section className="mt-5 rounded-2xl border border-white/[.08] bg-white/[.018] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><Filter className="h-4 w-4 text-cyan-300" /><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-cyan-300">DISPLAY FILTERS</p><p className="text-[11px] text-slate-500">Presentation only · server qualification and ranking are unchanged.</p></div></div><Button type="button" variant="outline" onClick={openMobileFilters} className="min-h-11 border-cyan-300/20 bg-cyan-300/[.05] text-[10px] text-cyan-100 lg:hidden"><SlidersHorizontal className="mr-2 h-4 w-4" />FILTERS{hasActiveFeedFilters(feedFilters) ? " · ACTIVE" : ""}</Button></div>{filterSummary.length ? <div className="mt-3 flex flex-wrap items-center gap-2 lg:hidden" aria-label="Active Opportunity Feed filters"><span className="min-h-11 rounded-lg border border-cyan-300/20 bg-cyan-300/[.05] px-3 py-2 text-[10px] font-semibold uppercase tracking-[.12em] text-cyan-100">{filterSummary.length} FILTER{filterSummary.length === 1 ? "" : "S"}</span>{filterSummary.map(filter => <button key={filter.key} type="button" onClick={() => updateFeedFilters(filter.patch)} aria-label={`Remove filter ${filter.label}`} className="min-h-11 rounded-lg border border-white/[.1] bg-white/[.03] px-3 py-2 text-[10px] font-semibold uppercase tracking-[.1em] text-slate-300">{filter.label} <span aria-hidden="true">×</span></button>)}<Button type="button" variant="outline" onClick={resetFeedFilters} className="min-h-11 border-rose-300/20 px-3 text-[10px] text-rose-100">CLEAR ALL</Button></div> : null}<div className="mt-3 hidden flex-wrap gap-2 lg:flex" aria-label="Opportunity Feed display filters">{(["ALL", "QUALIFIED", "POTENTIAL", "WATCH", "NO TRADE", "DATA UNAVAILABLE", "SCALP", "SWING", "LONG", "SHORT", "HEALTHY", "WARNING", "DATA LIMITED", "RISK OFF", "WATCHLIST ONLY"] as FilterValue[]).map(value => <Button key={value} type="button" variant="outline" aria-pressed={chipActive(value)} onClick={() => applyChipFilter(value)} className={`min-h-11 border-white/[.1] px-3 text-[10px] ${chipActive(value) ? "bg-cyan-300/[.12] text-cyan-100" : "bg-white/[.02] text-slate-400"}`}>{value}</Button>)}</div><p className="mt-3 text-[11px] text-slate-500" role="status" aria-live="polite">Showing {filteredItems.length} opportunities. {hasActiveFeedFilters(feedFilters) ? "Filters are display-only." : "Server ranking is unchanged."}</p></section>
    ...
    <Sheet open={quickViewItem !== null} onOpenChange={open => { if (!open) setQuickViewItem(null); }}><SheetContent side="right" className="w-full overflow-y-auto border-white/[.1] bg-[#0b1321] text-slate-100 sm:max-w-xl"><SheetHeader><SheetTitle className="text-slate-100">QUICK VIEW · {quickViewItem?.symbol ?? "OPPORTUNITY"}</SheetTitle><SheetDescription className="text-slate-400">Read-only server-derived decision preview. Open full analysis for the complete technical workspace.</SheetDescription></SheetHeader>{quickViewItem ? <div className="space-y-3 px-4"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase ${statusTone[quickViewItem.status] ?? "border-white/10 text-slate-300"}`}>{quickViewItem.status}</span><span className="rounded-full border border-white/[.1] px-2.5 py-1 text-[10px] uppercase text-slate-400">{quickViewItem.direction} · {quickViewItem.strategy} · {String(quickViewItem.timeframes?.execution ?? "UNAVAILABLE").toUpperCase()}</span></div><section className="rounded-xl border border-cyan-300/15 bg-cyan-300/[.04] p-4"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-cyan-200">WHY THIS SETUP?</p><p className="mt-2 text-sm leading-6 text-slate-300">{quickViewItem.exactReason ?? quickViewItem.whyInteresting?.[0] ?? "NOT AVAILABLE"}</p></section><section className="rounded-xl border border-violet-300/15 bg-violet-300/[.035] p-4"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-violet-200">PLAN</p><p className="mt-2 text-xs leading-5 text-slate-400">{quickViewItem.readinessPlan?.availability && quickViewItem.readinessPlan.availability !== "UNAVAILABLE" ? "Validated server-derived plan levels are available in full analysis." : "PLAN UNAVAILABLE — no valid level is returned by the server."}</p></section><section className="rounded-xl border border-rose-300/15 bg-rose-300/[.035] p-4"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-rose-200">RISK / WARNING</p><p className="mt-2 text-xs leading-5 text-slate-300">{warningFor(quickViewItem) ?? "No additional warning returned by the server."}</p></section><section className="rounded-xl border border-white/[.08] bg-white/[.02] p-4"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-slate-300">DATA</p><p className="mt-2 text-xs leading-5 text-slate-400">{quickViewItem.provider ?? "Provider unavailable"} · {quickViewItem.freshness ?? "Freshness unavailable"} · {quickViewItem.validationStatus ?? "Validation unavailable"}</p><p className="mt-1 text-[11px] text-slate-500">Last validated: {displayDate(quickViewItem.dataTimestamp)}</p></section></div> : null}<SheetFooter className="grid grid-cols-1 gap-2 sm:grid-cols-2"><Button type="button" variant="outline" onClick={() => { const item = quickViewItem; setQuickViewItem(null); if (item) onOpenDecision(item.assetId, returnContext); }} className="min-h-11">VIEW DECISION</Button><Button type="button" onClick={() => { const item = quickViewItem; setQuickViewItem(null); if (item) onInspectAsset(item.assetId, returnContext); }} className="min-h-11 bg-cyan-300 text-slate-950 hover:bg-cyan-200">OPEN FULL ANALYSIS</Button></SheetFooter></SheetContent></Sheet>
...
function SummaryMetric({ label, value, tone = "muted" }: { label: string; value: number; tone?: "positive" | "amber" | "cyan" | "risk" | "muted" }) {
  const valueTone = tone === "positive" ? "text-emerald-200" : tone === "amber" ? "text-amber-200" : tone === "risk" ? "text-rose-200" : tone === "cyan" ? "text-cyan-200" : "text-slate-100";
  return <div className="rounded-xl border border-white/[.07] bg-white/[.02] p-3"><p className="text-[9px] font-semibold uppercase tracking-[.12em] text-slate-500">{label}</p><p className={`mt-1 font-mono text-lg font-semibold ${valueTone}`}>{value}</p></div>;
}
