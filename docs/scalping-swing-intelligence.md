...
## Phase 18B — Production Exposure, Equity Snapshot Completion & Verification

Phase 18B completes exposure of the already-authorized Equity Snapshot layer. The existing `autoPaperEquitySnapshots` table is reused; no duplicate table or migration is created. It remains owner-scoped through `userId` and `accountId`, append-only and immutable, with the established five-minute account/time-bucket deduplication key, `(userId, capturedAt)` and `(accountId, capturedAt)` indexes, provenance, freshness, and server-derived account values.

The protected tRPC API now exposes `crypto.autoPaperEquitySnapshots`, `crypto.autoPaperEquityHistory`, and `crypto.autoPaperEquitySummary` under both existing `/api/trpc` and `/api/v1/trpc` middleware paths. History returns only persisted points and reports `NOT_INITIALIZED`, `NO_SNAPSHOTS`, or `READY`. Summary distinguishes account equity from trial performance and never creates a default account or synthetic historical point. The Lab consumes the history and summary contracts and shows an honest empty state.

Snapshot persistence remains attached to the approved server refresh/cron authority. No browser polling timer, new scheduler, automatic trial, Paper Trade, real order, provider-policy change, scoring change, or Manual Paper mutation was introduced. When no account or snapshot exists, the curve is empty rather than a fabricated starting/current line. Existing curve fallback points identify their historical or current provenance.

Phase 18B local validation passed with the existing schema/table and zero Auto Paper accounts or snapshots; Manual Paper counts remained unchanged at two portfolios and two trades during the read-only database check. Unauthenticated protected requests returned HTTP 401. The previously authenticated owner session returned HTTP 200 for account, performance, curve, and history; the older published runtime returned 404 for the three newly exposed procedures before this checkpoint. A post-checkpoint production propagation check is required before claiming final production reachability.
