...
describe("Research Lab immutable experiment persistence", () => {
  ...
  it("persists exact configuration, source provenance, result snapshot, and segmented result rows", async () => {
    ...
    const result = await runResearchExperiment(7, { name: "Persistence study", experimentId: "A", assetIds: ["bitcoin"], timeframe: "1h", candleLimit: 280, minimumOpportunity: 60, minimumConfidence: 60, holdingBars: 8, riskPercent: 1, stopAtrMultiplier: 1.5, takeProfitRule: "holding-close", targetRiskReward: 2, trainPercent: 70, regime: "ALL" }, structuredClone(DEFAULT_SCORING_CONFIG));
    const experimentInsert = inserted.find(entry => entry.table === researchExperiments)?.values as { configurationFingerprint: string; configuration: { input: { experimentId: string }; scoringConfiguration: unknown }; dataProvenance: unknown };
