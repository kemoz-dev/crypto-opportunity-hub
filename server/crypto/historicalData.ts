...
export type HistoricalScope = { datasetId: number; assetId: string; exchange: string; provider: string; instrumentType: InstrumentType; timeframe: Timeframe; scheduleExecutionId?: number; retryAttempt?: number };
...
export async function ingestHistoricalCandleBatch(scope: HistoricalScope, runKind: "backfill" | "incremental", candles: HistoricalCandleInput[], requestedStartAt?: number, requestedEndAt?: number, sourceDetails?: Record<string, unknown>) {
  ...
  await db.insert(historicalIngestionRuns).values({ batchId, datasetId: scope.datasetId, scheduleExecutionId: scope.scheduleExecutionId ?? null, retryAttempt: scope.retryAttempt ?? 0, runKind, status: "running", provider: scope.provider, exchange: scope.exchange, instrumentType: scope.instrumentType, assetId: scope.assetId, timeframes: [scope.timeframe], requestedStartAt: requestedStartAt ? new Date(requestedStartAt) : null, requestedEndAt: requestedEndAt ? new Date(requestedEndAt) : null, insertedCount: 0, duplicateCount: 0, malformedCount: audit.malformed.length, missingIntervalCount: audit.gaps.reduce((sum, gap) => sum + gap.expectedMissingCount, 0), details: { validation: { malformed: audit.malformed.map(item => item.reason), internalDuplicates: audit.internalDuplicates.length }, sourceDetails: sourceDetails ?? null } });
  ...
  try {
    const priorIssues = await findOpenScopeIssues(scope);
    const retryAttempt = scope.retryAttempt ?? Math.max(0, ...priorIssues.map(issue => issue.nextRetryAttempt));
    await appendHistoricalIssueEvent(priorIssues.map(issue => issue.id), { scheduleExecutionId: scope.scheduleExecutionId, ingestionRunId: run.id, eventType: "RETRY_STARTED", retryAttempt, details: { batchId, requestedStartAt, requestedEndAt } });
    ...
    const issueScope = { datasetId: scope.datasetId, scheduleExecutionId: scope.scheduleExecutionId, ingestionRunId: run.id, assetId: scope.assetId, exchange: scope.exchange, provider: scope.provider, instrumentType: scope.instrumentType, timeframe: scope.timeframe, expectedStartAt: requestedStartAt, expectedEndAt: requestedEndAt, actualStartAt: audit.valid[0]?.openTime, actualEndAt: audit.valid.at(-1)?.closeTime };
    if (!audit.valid.length) await recordHistoricalIngestionIssue(issueScope, "NO_NEW_CANDLES", "No new candles were available from the requested public archive interval.", 0, { batchId, sourceDetails: sourceDetails ?? null, duplicateCount: audit.internalDuplicates.length });
    if (allCandleAudit.gaps.length) await recordHistoricalIngestionIssue(issueScope, "MISSING_RANGE", "Observed candle continuity contains unresolved intervals.", missingCandles, { batchId, gaps: allCandleAudit.gaps, sourceDetails: sourceDetails ?? null });
    if (audit.valid.length && !allCandleAudit.gaps.length) await appendHistoricalIssueEvent(priorIssues.map(issue => issue.id), { scheduleExecutionId: scope.scheduleExecutionId, ingestionRunId: run.id, eventType: "RETRY_SUCCEEDED", retryAttempt, details: { batchId, insertedCount: fresh.length, duplicateCount: audit.internalDuplicates.length + (audit.valid.length - fresh.length) } });
