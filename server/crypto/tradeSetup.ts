...
function buildLevels(direction: TradeSetupDirection, candles: Candle[], currentPrice: number, atr: number) {
  ...
  if (stopPrice === null || (direction === "LONG" ? stopPrice >= entryLow : stopPrice <= entryHigh)) return null;
  ...
  if (!Number.isFinite(rr) || rr < 1) return null;
  return {
...
export function buildTradeSetupPlan(mode: TradeSetupMode, row: ScannerRow, regime: MarketRegime | null, candles: Candle[], provider: string, timestamp: number): TradeSetupPlan {
  ...
  if (!row.score || row.asset.price === null) return emptyPlan(mode, row, regime, "Validated live price and Opportunity inputs are required.");
  if (!execution || !confirmation || !context) return emptyPlan(mode, row, regime, "Required validated multi-timeframe analysis is unavailable.");
  const direction = directionFrom(row.score, regime);
  if (direction === "NO TRADE") return { ...emptyPlan(mode, row, regime, regime?.classification === "RISK OFF" ? "Market context is hostile; no actionable plan is presented." : "Existing Opportunity direction is neutral; no trade plan is presented.", "LIVE"), executionState: execution, confirmationState: confirmation, contextState: context, dataTimestamp: timestamp, provider };
  const atr = calculateAtr(candles);
  if (atr === null || atr <= 0) return { ...emptyPlan(mode, row, regime, "ATR is unavailable on the validated execution timeframe, so technical levels cannot be derived."), executionState: execution, confirmationState: confirmation, contextState: context, dataTimestamp: timestamp, provider };
  const levels = buildLevels(direction, candles, row.asset.price, atr);
  if (!levels) return { ...emptyPlan(mode, row, regime, "A structurally valid stop and at least one positive risk/reward target could not be derived; NO TRADE."), executionState: execution, confirmationState: confirmation, contextState: context, dataTimestamp: timestamp, provider };
...
export async function getTradeSetups(mode: TradeSetupMode, configuration: ScoringConfig) {
  ...
  return { mode, generatedAt: scan.generatedAt, lowerTimeframeDataReady: LOWER_TIMEFRAME_DATA_READY, minimumValidatedTimeframe: PROFILES[mode].minimumLabel, marketRegime: scan.marketRegime, setups: setups.sort((left, right) => Number(right.actionable) - Number(left.actionable) || (right.tradeSetupQuality ?? -1) - (left.tradeSetupQuality ?? -1) || (right.opportunityScore ?? -1) - (left.opportunityScore ?? -1)) };
}
...
export async function getTradeSetupForRow(mode: TradeSetupMode, row: ScannerRow, regime: MarketRegime | null, configuration: ScoringConfig, minimumCandles = Math.max(configuration.indicator.emaSlow + 2, configuration.indicator.macdSlow + configuration.indicator.macdSignal + 2, 60)) {
  ...
  if (!ohlcv.series || (expectedProvider !== null && expectedProvider !== ohlcv.series.provider)) return emptyPlan(mode, row, regime, expectedProvider ? "Execution OHLCV provider coherence could not be verified." : "Validated execution OHLCV is unavailable.");
