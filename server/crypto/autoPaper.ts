...
export async function getAutoPaperPerformance(userId: number, filters?: AutoPaperPerformanceFilters) {
  ...
  const exitIntelligence = [terminalBy("TARGET_1", trial => trial.status === "TARGET_1_REACHED"), terminalBy("TARGET_2", trial => trial.status === "TARGET_2_REACHED"), terminalBy("TARGET_3", trial => trial.status === "TARGET_3_REACHED"), terminalBy("STOP_LOSS", trial => trial.status === "STOPPED"), terminalBy("INVALIDATED", trial => trial.status === "INVALIDATED"), terminalBy("DATA_UNAVAILABLE", trial => trial.status === "DATA_UNAVAILABLE"), terminalBy("OPEN", trial => ACTIVE_TRIAL_STATUSES.includes(trial.status as typeof ACTIVE_TRIAL_STATUSES[number]))];
