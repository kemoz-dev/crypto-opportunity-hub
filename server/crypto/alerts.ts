...
export async function evaluateAlert(alertId: number, expectedTaskUid?: string) {
  ...
  const snapshot = { generatedAt: scan.generatedAt, conditions, marketRegime: scan.marketRegime, matches: matches.map(row => ({ asset: { id: row.asset.id, symbol: row.asset.symbol, name: row.asset.name, price: row.asset.price, sector: row.asset.sector }, score: row.score ? { opportunity: row.score.score, confidence: row.score.confidence, technical: row.score.technicalScore, setup: row.score.setupType, confirmingSignals: row.score.reasons.filter(reason => reason.direction === "positive").slice(0, 4), risks: row.score.missingConditions } : null, dataStatus: row.dataStatus })) };
  const db = await getDb();
