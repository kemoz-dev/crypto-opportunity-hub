...
describe("Disaster Recovery protected routes", () => {
  it("rejects unauthenticated archive list, primary lookup, creation, and download access", async () => {
    ...
    await expect(caller.crypto.verifiedPrimaryRecoveryArchive()).rejects.toMatchObject({ code: "UNAUTHORIZED" });
    ...
    await expect(caller.crypto.downloadVerifiedPrimaryRecoveryArchive()).rejects.toMatchObject({ code: "UNAUTHORIZED" });
  });
});
