...
import { autoPaperSettingsSchema, evaluateAndCreateAutoPaperTrial, getAutoPaperAccount, getAutoPaperActive, buildAutoPaperReport, getAutoPaperEquityCurve, getAutoPaperEventFeed, getAutoPaperEvents, getAutoPaperHistory, getAutoPaperPerformance, getAutoPaperSettings, recordAutoPaperEvent, refreshAutoPaperActive, refreshAutoPaperForAllEnabled, saveAutoPaperSettings } from "../crypto/autoPaper";
...
export const cryptoRouter = router({
  ...
  autoPaperEquityCurve: protectedProcedure.input(z.object({ strategy: z.string().max(64).optional(), timeframe: z.string().max(12).optional(), direction: z.enum(["long", "short"]).optional(), mode: z.string().max(24).optional(), assetId: z.string().max(96).optional(), regime: z.string().max(32).optional(), qualification: z.string().max(32).optional(), from: z.number().optional(), to: z.number().optional() }).optional()).query(({ ctx, input }) => getAutoPaperEquityCurve(ctx.user.id, input)),
  autoPaperExport: protectedProcedure.input(z.object({ format: z.enum(["json", "csv"]), filters: z.object({ strategy: z.string().max(64).optional(), timeframe: z.string().max(12).optional(), direction: z.enum(["long", "short"]).optional(), mode: z.string().max(24).optional(), assetId: z.string().max(96).optional(), regime: z.string().max(32).optional(), qualification: z.string().max(32).optional(), from: z.number().optional(), to: z.number().optional() }).optional() })).query(({ ctx, input }) => buildAutoPaperReport(ctx.user.id, input.filters)),
