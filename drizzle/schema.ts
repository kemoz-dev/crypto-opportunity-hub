...
export const autoPaperAccounts = mysqlTable("autoPaperAccounts", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  name: varchar("name", { length: 96 }).notNull().default("Auto Paper Simulation"),
  startingCapital: double("startingCapital").notNull().default(100000),
  currentEquity: double("currentEquity").notNull().default(100000),
  availableCash: double("availableCash").notNull().default(100000),
  realizedPnl: double("realizedPnl").notNull().default(0),
  unrealizedPnl: double("unrealizedPnl").notNull().default(0),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [uniqueIndex("auto_paper_account_user_unique").on(table.userId)]);

export const autoPaperSettings = mysqlTable("autoPaperSettings", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  enabled: boolean("enabled").notNull().default(false),
  mode: mysqlEnum("mode", ["CONSERVATIVE", "BALANCED", "OPPORTUNITY", "EXPERIMENTAL", "CUSTOM"]).notNull().default("BALANCED"),
...
export const autoPaperTrials = mysqlTable("autoPaperTrials", {
  ...
  paperTradeId: int("paperTradeId").references(() => paperTrades.id, { onDelete: "set null" }),
  ...
  mode: mysqlEnum("mode", ["CONSERVATIVE", "BALANCED", "OPPORTUNITY", "EXPERIMENTAL", "CUSTOM"]).notNull().default("BALANCED"),
  status: mysqlEnum("status", ["DETECTED", "ENTERED", "OPEN", "HEALTHY", "TARGET_1_REACHED", "TARGET_2_REACHED", "TARGET_3_REACHED", "WARNING", "REVERSAL_RISK", "INVALIDATED", "STOPPED", "CLOSED", "COMPLETED", "DATA_UNAVAILABLE", "EXPIRED"]).notNull().default("DETECTED"),
  ...
  rewardRisk: double("rewardRisk").notNull(),
  positionSize: double("positionSize").notNull().default(0),
  riskPercent: double("riskPercent").notNull().default(1),
  realizedPnl: double("realizedPnl").notNull().default(0),
  currentPnl: double("currentPnl").notNull().default(0),
  provider: varchar("provider", { length: 96 }).notNull(),
