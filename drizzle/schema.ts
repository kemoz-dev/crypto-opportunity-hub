...
export const executionCostModels = mysqlTable("executionCostModels", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  name: varchar("name", { length: 128 }).notNull(),
  version: varchar("version", { length: 96 }).notNull(),
  ...
  configuration: json("configuration").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => [uniqueIndex("execution_cost_model_user_fingerprint_unique").on(table.userId, table.configurationFingerprint), index("execution_cost_model_user_created_idx").on(table.userId, table.createdAt)]);

export const historicalFundingRates = mysqlTable("historicalFundingRates", {
  id: int("id").autoincrement().primaryKey(),
  datasetId: int("datasetId").notNull().references(() => historicalDatasets.id, { onDelete: "cascade" }),
  assetId: varchar("assetId", { length: 96 }).notNull().references(() => assets.id, { onDelete: "cascade" }),
  exchange: varchar("exchange", { length: 64 }).notNull(),
  ...
  rateType: varchar("rateType", { length: 32 }),
  fundingIntervalMs: bigint("fundingIntervalMs", { mode: "number" }),
  intervalEvidence: varchar("intervalEvidence", { length: 64 }).notNull(),
  dataQuality: mysqlEnum("dataQuality", ["AVAILABLE", "PARTIAL", "UNAVAILABLE"]).notNull(),
  source: varchar("source", { length: 128 }).notNull(),
  sourcePayload: json("sourcePayload"),
  retrievedAt: timestamp("retrievedAt").defaultNow().notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => [uniqueIndex("historical_funding_dataset_asset_source_time_unique").on(table.datasetId, table.assetId, table.source, table.fundingTime), index("historical_funding_lookup_idx").on(table.datasetId, table.assetId, table.fundingTime)]);

export const historicalLiquidityObservations = mysqlTable("historicalLiquidityObservations", {
  id: int("id").autoincrement().primaryKey(),
  datasetId: int("datasetId").notNull().references(() => historicalDatasets.id, { onDelete: "cascade" }),
  assetId: varchar("assetId", { length: 96 }).notNull().references(() => assets.id, { onDelete: "cascade" }),
  exchange: varchar("exchange", { length: 64 }).notNull(),
  ...
  quoteVolume: double("quoteVolume"),
  marketCap: double("marketCap"),
  volumeMarketCapRatio: double("volumeMarketCapRatio"),
  liquidityTier: mysqlEnum("liquidityTier", ["A", "B", "C", "D", "E", "UNAVAILABLE"]).notNull(),
  dataQuality: mysqlEnum("dataQuality", ["AVAILABLE", "PARTIAL", "UNAVAILABLE"]).notNull(),
  source: varchar("source", { length: 128 }).notNull(),
  evidence: json("evidence").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => [uniqueIndex("historical_liquidity_dataset_scope_time_unique").on(table.datasetId, table.assetId, table.exchange, table.instrumentType, table.timeframe, table.observedAt), index("historical_liquidity_dataset_asset_idx").on(table.datasetId, table.assetId, table.createdAt)]);

export const executionCostStudies = mysqlTable("executionCostStudies", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  modelId: int("modelId").references(() => executionCostModels.id),
  name: varchar("name", { length: 128 }).notNull(),
  status: mysqlEnum("status", ["running", "completed", "failed"]).notNull(),
  datasetId: int("datasetId").notNull().references(() => historicalDatasets.id),
  ...
  timeframe: mysqlEnum("timeframe", ["15m", "1h", "4h", "1d"]).notNull(),
  entryAt: timestamp("entryAt").notNull(),
  exitAt: timestamp("exitAt").notNull(),
  tradeSizeUsd: double("tradeSizeUsd").notNull(),
  grossEntryPrice: double("grossEntryPrice").notNull(),
  grossExitPrice: double("grossExitPrice").notNull(),
  grossReturnPercent: double("grossReturnPercent").notNull(),
  configurationFingerprint: varchar("configurationFingerprint", { length: 128 }).notNull(),
  configuration: json("configuration").notNull(),
  dataProvenance: json("dataProvenance").notNull(),
  resultSnapshot: json("resultSnapshot"),
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  completedAt: timestamp("completedAt"),
}, table => [index("execution_cost_study_user_created_idx").on(table.userId, table.createdAt), index("execution_cost_study_dataset_asset_idx").on(table.datasetId, table.assetId, table.createdAt), index("execution_cost_study_status_idx").on(table.status, table.createdAt)]);
