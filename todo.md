...
- [ ] Audit and document the complete authorized Tauri Windows POC and Opportunities Intelligence scope, existing technical inputs, provider/timeframe limits, authentication constraints, and no-regression boundaries.
- [ ] Add a least-privilege Tauri Windows proof of concept that loads only the approved Crypto Hub HTTPS origin, has no filesystem/shell/process privileges, blocks arbitrary navigation, and uses the shared frontend/backend without secrets or a local trade store.
- [ ] Define and implement a separate explainable Setup Intelligence and risk/reward layer for validated scalping and swing inputs without modifying Opportunity/Regime scoring, thresholds, providers, Paper Trading rules, or real-trading prohibition.
- [ ] Add Opportunities navigation, Scalping/Swing setup workspaces, multi-timeframe evidence, risk/target/invalidation states, server-authoritative simulated trade monitoring, and responsive web/desktop UX without fabricated levels or probabilities.
- [ ] Add deterministic security, methodology, regression, and desktop configuration tests; run complete validation and publish only after reporting any authentication or desktop-build blockers honestly.
