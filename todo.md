...
- [ ] Add deterministic Phase 4 setup, target, Trade Health, snapshot-immutability, provider-coherence, PWA/security, and protected-regression tests; run full tests, TypeScript, production build, secret/cache scans, controlled production smoke validation, document exact outcomes, and publish only when checks pass.
