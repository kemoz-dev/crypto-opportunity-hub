...
- [ ] Run full tests, TypeScript, production build, static secret/cache checks, local browser-class offline reload/online-recovery checks, mobile/landscape/tablet screenshots, and read-only production smoke validation; document Safari/device limits truthfully and publish only when checks pass.
